...
const SSO_BASE_URL = resolveSSOBaseUrl();
...
async function refreshAccessToken() {
    ...
    refreshPromise = (async () => {
        try {
            ...
            console.log('[Token Refresh] Using refresh token (masked):', maskToken(refreshToken));
            ...
            try {
                ...
                console.warn('[Token Refresh] Failed to decode token payload, using existing user data:', decodeError);
            ...
            await secureTokenStorage.setTokens(accessToken, newRefreshToken, updatedUser ?? undefined);
            // Broadcast token refresh event to other tabs/windows
            if (typeof window !== 'undefined' && window.localStorage) {
                window.localStorage.setItem('icaptur_token_refresh', JSON.stringify({ timestamp: Date.now(), accessToken }));
            ...
            if (axiosError.response?.status === 401 ||
                ...
                console.warn('[Token Refresh] Refresh token invalid/expired, logging out user');
                // Clear tokens immediately
                secureTokenStorage.clearTokens();
                // Broadcast logout event to all tabs (this will trigger logout in all apps)
                if (typeof window !== 'undefined' && window.localStorage) {
                    window.localStorage.setItem('icaptur_logout', JSON.stringify({ timestamp: Date.now() }));
                ...
                if (typeof window !== 'undefined') {
                    ...
                    if (typeof window !== 'undefined') {
                        ...
                        if (win.__SSO_APP_URL__) {
                            ...
                            ssoAppUrl =
                                import.meta.env?.VITE_SSO_APP_URL ?? '';
                        }
                    }
                    if (ssoAppUrl) {
                ...
                if (newToken) {
                    try {
                        ...
                        if (payload.sub && payload.email && payload.role) {
                            ...
                            const refreshToken = await secureTokenStorage.getRefreshToken();
                            await secureTokenStorage.setTokens(newToken, refreshToken ?? undefined, updatedUser);
                        }
                    }
                    catch (decodeError) {
                        console.warn('[Cross-Tab Sync] Failed to decode token payload:', decodeError);
                    }
                }
            }
            catch {
 ...
 */
export async function centralizedLogout() {
    const refreshToken = await secureTokenStorage.getRefreshToken();
    ...
    if (refreshToken) {
        try {
            ...
            await axios.post(`${SSO_BASE_URL_LOCAL}/auth/logout`, {
                refreshToken,
            }, {
                headers: {
                    'Content-Type': 'application/json',
    ...
    try {
        const { forceClearAuthStorage } = await import('../utils/secureTokenStorage');
 ...
 */
function resolveSSOAppUrl() {
    if (typeof window !== 'undefined') {
        const win = window;
        if (win.__SSO_APP_URL__)
            return win.__SSO_APP_URL__;
    }
    return import.meta.env?.VITE_SSO_APP_URL ?? '';
}
/**
 ...
 */
function resolveSSOBaseUrl() {
    let baseUrl = import.meta.env?.VITE_SSO_API_URL ?? '';
    if (typeof window !== 'undefined') {
        const win = window;
